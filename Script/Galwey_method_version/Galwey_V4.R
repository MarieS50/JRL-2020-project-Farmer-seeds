library(anyLib)
anyLib(c("data.table", "apercu", "corpcor","plyr"))
...
dim(geno)
  ...
  # geno ̩tant la matrice avec les individus en ligne et les SNPs en colonne.
...
LDmat <- as.matrix(snpStats::ld(test[,1:86000], depth = ncol(test[,1:86000]) - 1, stats = "R"))
  
  # LDMat is a square matrix  with only the UPPER part filled 
  # need to symetrize it
  
LDmat[lower.tri(LDmat)] = t(LDmat)[lower.tri(LDmat)] # on compl̬te la partie inf̩rieure
diag(LDmat) <- 1 # on compl̬te la diagonale
  

# on the whole set of chromosomes
Me<-meff(LDmat, method = "galwey")
